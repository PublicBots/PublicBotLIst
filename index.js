...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[publicbots.eu]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.on('ready', async () => {
    console.log("[publicbots.eu]: Bot successfully connected as " + client.user.tag + ".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'publicbots.eu | ' + bots.length + ' bots' } }); // , status: "dnd" 
});
/*=======================================================================================*/
