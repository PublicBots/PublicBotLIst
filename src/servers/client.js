...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[bot-list.publicbots.eu/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[bot-list.publicbots.eu/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    serverClient.user.setPresence({ activity: { type: 'WATCHING', name: 'b!bump & b!vote | bot-list.publicbots.eu/servers' }, status: "online" });
});
