...
console.log("[publicbots.eu]: Botlist/Bot view router loaded.");
app.get("/bot/:botID", async (req, res, next) => {
    ...
    if (botdata.status != "Approved") {
        ...
        if (req.user.id == botdata.ownerID || botdata.coowners.includes(req.user.id) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.administrator) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.moderator)) {
