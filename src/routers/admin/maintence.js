...
const maintenceSchema = require("../../database/models/maintence");
const client = global.Client;
const channels = global.config.server.channels,
	  roles = global.config.server.roles;

console.log("[bot-list.publicbots.eu]: Admin/Maintence router loaded.");
...
app.post("/admin/maintence", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.webstatus).send(`<a:Dis_off:850922384080240640>  has been switched to __Maintenance__ due to **${req.body.reason}** [||<@&844914717377691678>||]`).then(a => {
        new maintenceSchema({
            server: config.server.id,
...
app.post("/admin/unmaintence", global.checkAuth, async (req, res) => {
    ...
    const bakimsonaerdikardesvcodes = new dc.MessageEmbed()
        .setAuthor("bot-list.publicbots.eu", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`  are **active** again!\n[Click to redirect website](https://bot-list.publicbots.eu)`)
        .setFooter(" © All rights reserved.");
    await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
        a.edit(`~~   has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesvcodes)
    })
